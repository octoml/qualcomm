         ...
         const int iters)
{
    ...
    tmpA[1] = (*((__global float4*)&matrixA[index]));
    tmpB[0] = (*((__global float4*)&matrixB[index]));
    tmpB[1] = (*((__global float4*)&matrixB[index]));
    for (int i = 0; i < iters; ++i) {
        tmpA[0].w= MAD(tmpA[0].w, tmpA[0].w, tmpB[0].w);
        tmpA[0].x = MAD(tmpA[0].x, tmpA[0].x, tmpB[0].x);
        tmpA[0].y = MAD(tmpA[0].y, tmpA[0].y, tmpB[0].y);
        tmpA[0].z = MAD(tmpA[0].z, tmpA[0].z, tmpB[0].z);
        tmpA[1].w = MAD(tmpB[1].w, tmpA[1].w, tmpB[1].w);
        tmpA[1].x = MAD(tmpB[1].x, tmpA[1].x, tmpB[1].x);
        tmpA[1].y = MAD(tmpB[1].y, tmpA[1].y, tmpB[1].y);
        tmpA[1].z = MAD(tmpB[1].z, tmpA[1].z, tmpB[1].z);
    }
    (*((__global float4*)&MatrixSum[index])) = MAD(tmpA[0], tmpA[0], tmpB[0]);
    (*((__global float4*)&MatrixSum[index])) = MAD(tmpA[1], tmpA[1], tmpB[1]);
}
