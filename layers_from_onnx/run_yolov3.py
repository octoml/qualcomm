...
class TestYolov3():
    def __init__(self):
        ...
        self.dtype = "float32"
        self.stat_file = f"yolov3_{self.dtype}.autotvm.log"
    ...
    def run_module_nms(self, module, dtype, params, input_shape, dev):
        module.set_input(**params)
        inp = np.random.normal(size=input_shape).astype(dtype)
    ...
    def test_run(self):
        ...
        for boxes_shape, scores_shape, max_output_boxes_per_class, iou_threshold, score_threshold in self.nms_data:
            ...
            graph, lib, params = self.build_model_with_stat(mod, params, self.stat_file)
            rlib, module, dev = self.create_module(graph, lib, debug=False)
            self.run_module_nms(module, self.dtype, params, boxes_shape, dev)
            del dev
            del module
            del rlib
            self.kill()
