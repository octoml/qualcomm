...
def compute_conv2d_NCHWc_KCRSk_tx_acc32(Input, Filter, stride, padding, dilation, out_dtype=None):
    ...
    if args.memory != None:
        ...
        output = te.compute(conv.shape, lambda n,fc,y,x,fb: conv[n,fc,y,x,fb].astype(args.dtype))
    else:
        conv = te.compute(
            (batch, num_filter_chunk, out_height, out_width, num_filter_block),
            lambda nn, ffc, yy, xx, ffb: te.sum(
...
def test_texture(target="opencl", target_host="llvm -mtriple=arm64-linux-android"):
    if args.test == "plus_one_rank3":
        ...
        elif args.test == "conv2d_NCHWc_KCRSk_tx_tune":
            # NCHWc, KCRSk
            input_shape, filter_shape = (1, 32, 56, 56, 4), (32, 128, 1, 1, 4)
            s, placeholders = tune_and_bench(conv2d_NCHWc_KCRSk_tx_template, args.test, input_shape, filter_shape)
        elif args.test == "conv2d_NCHWc_KCRSk_tx_fp32acc_tune":
            # NCHWc, KCRSk
            input_shape, filter_shape = (1, 32, 56, 56, 4), (32, 128, 1, 1, 4)
            s, placeholders = tune_and_bench(conv2d_NCHWc_KCRSk_tx_fp32acc_template, args.test, input_shape, filter_shape)
